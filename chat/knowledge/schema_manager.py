...
class SchemaManager:
    ...
    def generate_compact_schema_summary(self) -> str:
        """컴팩트한 스키마 요약 생성 (GPT-5 컨텍스트용)"""
        summary_parts = []
        
        summary_parts.append("# 📊 Database Schema Overview\n")
        ...
        for db_name, schema in self.schemas.items():
            ...
            summary_parts.append(f"*Last updated: {extracted_at[:10]}*\n")
            
            # 주요 테이블만 요약 (행 수 기준 상위 10개)
            ...
            for table_name, table_info in sorted_tables:
                ...
                summary_parts.append(f"  - {col_count} columns, {total_rows:,} rows")
                
                # 핵심 컬럼 몇 개만 표시
                columns = table_info.get('columns', {})
                key_columns = []
                for col_name, col_info in list(columns.items())[:5]:  # 처음 5개만
                    col_type = col_info.get('type', '').split('(')[0]  # 타입 간소화
            ...
            summary_parts.append("")  # 데이터베이스 간 구분
